...
test("action reason absent from complete canonical set is rejected",()=>{const x=staleHigh();x.next_action_reason_codes=["RSN-COVERAGE-INCOMPLETE"];x.reason_codes=["RSN-COVERAGE-INCOMPLETE"];assert.throws(()=>validateInput(x));});
...
test("stale-to-repair escalation is rejected",()=>{const x=staleHigh();x.action_kind="repair";x.prompt_kind="implementer_repair_prompt";x.recipient="implementer_model";x.max_modify_code=true;assert.throws(()=>validateInput(x));});
