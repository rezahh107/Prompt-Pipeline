...
export type TechnicalStatus = "GREEN_TECHNICALLY_READY" | "YELLOW_CHANGES_OR_VERIFICATION_REQUIRED" | "RED_DO_NOT_MERGE";
...
export interface RendererInput {
  ...
  pr_inspector_protocol_version: "v1.10.2"; canonical_review_package_sha256: string; review_validity: ReviewValidity;
  action_kind: ActionKind; prompt_kind: PromptKind; prompt_required: boolean; recipient: Recipient; may_modify_code: boolean;
  technical_status: TechnicalStatus; approval_requirement: ApprovalRequirement; reason_codes: string[]; reason_details: Array<{reason_code:string; subjects:string[]}>;
  required_actions: string[]; findings: Finding[]; repair_handoff: RepairHandoff | null; evidence_records: EvidenceRecord[];
  ...
  prompt_language: "en"; required_target_output_language: string; success_criteria: string[]; failure_modes: string[]; evaluation_suite_id: "pr_inspector_action.v1";
}
...
export interface RendererOutput { schema_version:"pr_inspector_action_output.v1"; prompt_required:boolean; action_kind:ActionKind; prompt_kind:PromptKind; recipient:Recipient; may_modify_code:boolean; technical_status:TechnicalStatus; approval_requirement:ApprovalRequirement; review_validity:ReviewValidity; rendered_prompt:string|null; rendered_prompt_sha256:string|null; canonical_input_sha256:string; generator:Record<string,unknown>; human_review:{required:boolean; approval_satisfied:false; boundary:string}; risk_metadata:{technical_status:TechnicalStatus; sensitive_domains:string[]} }
