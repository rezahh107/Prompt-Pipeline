...
function ciStep(step: string): boolean { const m = step.match(/^pnpm\s+([^\s]+)/); if (!m) return false; const script = m[1]; const pkg = load('package.json') as { scripts?: Record<string, string> }; const scripts = pkg.scripts ?? {}; return Boolean(script && scripts[script] && (script === 'ci' || (scripts.ci ?? '').includes(`pnpm ${script}`))); }
