...
function sortedJson(value: unknown): unknown {
  ...
  if (value !== null && typeof value === 'object') {
    const out: Dict = {};
    for (const key of Object.keys(value).sort()) out[key] = sortedJson((value as Dict)[key]);
    return out;
  }
  return value;
}
...
function hashJson(value: unknown): string {
  return sha256(JSON.stringify(sortedJson(value)) ?? 'undefined');
}
...
function caseShouldPass(caseFile: string): boolean {
  const data = yaml.load(readFileSync(caseFile, 'utf8')) as { expected?: { validation?: { should_pass?: boolean } } } | null;
  return data?.expected?.validation?.should_pass !== false;
}

function packageManager(): string | null {
  try {
    return (JSON.parse(readFileSync('package.json', 'utf8')) as { packageManager?: string }).packageManager ?? null;
  } catch {
    return null;
  }
}

function requireFileHash(failures: string[], label: string, path: string, expected: string): void {
  if (!existsSync(path)) {
    failures.push(`${label}: missing file ${path}`);
    return;
  }
  const actual = sha256(readFileSync(path, 'utf8'));
  if (actual !== expected) failures.push(`${label}: hash mismatch for ${path}`);
}

function assertArray(name: string, value: string[], failures: string[], label: string, allowEmpty = false): void {
  if (!Array.isArray(value)) failures.push(`${label}: ${name} is not an array`);
  else if (!allowEmpty && value.length === 0) failures.push(`${label}: ${name} is empty`);
}
...
try {
  ...
  failures.push(`request artifact: generation failed: ${(error as Error).message}`);
}

try {
  const scoredArtifact = generateArtifact({ request: 'review code for correctness and tests', mode: 'ci' }).artifact as Artifact;
  const evidence = scoredArtifact.provenance.routing_evidence;
  if (!evidence) failures.push('scored request artifact: missing routing_evidence');
  else {
    if (evidence.score <= 0) failures.push('scored request artifact: non-positive router score');
    if (evidence.matched_terms.length === 0) failures.push('scored request artifact: missing matched_terms');
    if (!Array.isArray(evidence.competing_candidates)) failures.push('scored request artifact: competing_candidates is not an array');
  }
  checkArtifact('scored request artifact', scoredArtifact, failures);
  checked += 1;
} catch (error) {
  failures.push(`scored request artifact: generation failed: ${(error as Error).message}`);
}
