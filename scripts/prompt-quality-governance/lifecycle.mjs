...
export async function validateLifecycle(l,s,c={}){
  const source=c.source||L;
  let z=await schema(LS,l,source);
  if(l.repository!==REPO||l.program_id!==PID||l.task_id!==s.task_id||l.base_sha!==s.base_sha)z.push(d('PQG_LIFECYCLE_IDENTITY_MISMATCH','ledger identity',source));
  ...
  for(let n=0;n<(l.events||[]).length;n++){
    const e=l.events[n];
    if(seen.has(e.event_id))z.push(d('PQG_LIFECYCLE_EVENT_REPLAY',e.event_id,source));
    seen.add(e.event_id);
    ...
    const currentTime=Date.parse(e.occurred_at),previousTime=n?Date.parse(l.events[n-1].occurred_at):null;
    if(Number.isNaN(currentTime)||(n&&(Number.isNaN(previousTime)||currentTime<previousTime)))z.push(d('PQG_LIFECYCLE_TIME_INVALID',e.event_id,source));
    ...
    if(e.scope_revision!==s.scope_revision||l.scope_revision!==s.scope_revision)z.push(d('PQG_LIFECYCLE_SCOPE_STALE',e.event_id,source));
    ...
    if(e.event_type==='exact_head_validated'){
      const receipt=receiptFor(c.evidenceIndex,l,e);
      if(e.pull_request==null||e.pull_request!==l.pull_request)z.push(d('PQG_LIFECYCLE_IDENTITY_MISMATCH','exact-head PR',source));
      if(e.evidence?.[0]?.kind!=='authoritative_exact_head_ci'||!receipt)z.push(d('PQG_EXACT_HEAD_EVIDENCE_MISSING','verified canonical producer receipt missing or mismatched',source));
      else if(c.head&&receipt.facts?.current_head!==c.head)z.push(d('PQG_LIFECYCLE_HEAD_STALE','validated SHA is not bound to the checked-out repository Head',source));
    }
    if(e.event_type==='owner_merge'){
      const receipt=receiptFor(c.evidenceIndex,l,e),validated=l.events[n-1];
      if(e.pull_request==null||e.pull_request!==l.pull_request)z.push(d('PQG_LIFECYCLE_IDENTITY_MISMATCH','owner-merge PR',source));
      if(e.evidence?.[0]?.kind!=='authoritative_owner_merge'||!receipt)z.push(d('PQG_OWNER_MERGE_EVIDENCE_MISSING','verified owner receipt missing or mismatched',source));
      else if(validated?.event_type!=='exact_head_validated'||receipt.facts?.pr_head_sha!==validated.head_sha||receipt.facts?.base_ref!==EVIDENCE_POLICY.default_branch||receipt.facts?.merged_by?.login!==EVIDENCE_POLICY.owner.login||receipt.facts?.merged_by?.id!==EVIDENCE_POLICY.owner.id)z.push(d('PQG_OWNER_MERGE_EVIDENCE_MISSING','target branch, merged PR Head or owner does not match lifecycle policy',source));
    }
...
export function completeLedger(base,s){const l=cp(base),h=['a'.repeat(40),'b'.repeat(40),'c'.repeat(40)],g=['1'.repeat(64),'2'.repeat(64),'3'.repeat(64)],k=['authoritative_exact_head_ci','authoritative_owner_merge','authoritative_exact_main'];let prev=l.events.at(-1).event_id,num=Number(prev.match(/(\d+)$/)?.[1]||0)+1;const receipts=[];for(const [n,t] of ['exact_head_validated','owner_merge','exact_main_verified'].entries()){const e={event_id:`PROMPT-QUALITY-EVENT-${String(num+n).padStart(4,'0')}`,event_version:1,event_type:t,predecessor_event_id:prev,occurred_at:`2026-07-18T12:25:${20+n}Z`,repository:REPO,program_id:PID,task_id:l.task_id,pull_request:29,base_sha:s.base_sha,head_sha:h[n],scope_revision:s.scope_revision,evidence:[{kind:k[n],reference:t,sha256:g[n]}],event_hash:''};reEvent(e);l.events.push(e);const descriptor={task_id:l.task_id,event_type:t,pull_request:29,base_sha:s.base_sha,subject_sha:h[n],scope_revision:s.scope_revision,evidence_digest:g[n]};if(t==='exact_head_validated')descriptor.facts={validated_sha:h[0],current_head:h[0],carrier_mode:'exact',changed_paths:[]};if(t==='owner_merge')descriptor.facts={pr_head_sha:h[0],merge_commit_sha:h[1],base_ref:EVIDENCE_POLICY.default_branch,merged_by:cp(EVIDENCE_POLICY.owner)};if(t==='exact_main_verified')descriptor.facts={branch:EVIDENCE_POLICY.default_branch,head_sha:h[2],owner_merge_commit_sha:h[1],contains_owner_merge:true,compare_status:'ahead',merge_base_sha:h[1]};receipts.push(descriptor);prev=e.event_id}l.pull_request=29;l.branch_kind='default';l.completion_claim=true;l.next_required_event=null;reLedger(l);return{l,h,g,receipts}}
