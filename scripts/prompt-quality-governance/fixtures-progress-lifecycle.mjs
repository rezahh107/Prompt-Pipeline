...
else if(c==='postmerge_complete_sequence'){const x=completeLedger(l,s);z.push(...await validateLifecycle(x.l,s,{head:x.h[0],main:x.h[2],headDigests:new Set([x.g[0]]),mergeDigests:new Set([x.g[1]]),requireEvidence:true}))}
else if(c==='owner_merge_without_evidence'){const x=completeLedger(l,s);x.l.events.pop();x.l.completion_claim=false;x.l.next_required_event='exact_main_verified';reLedger(x.l);z.push(...await validateLifecycle(x.l,s,{head:x.h[0],headDigests:new Set([x.g[0]]),mergeDigests:new Set(),requireEvidence:true}))}
else if(c==='exact_main_before_owner_merge'){const x=completeLedger(l,s);x.l.events.splice(-2,1);x.l.events.at(-1).predecessor_event_id=x.l.events.at(-2).event_id;reLedger(x.l);z.push(...await validateLifecycle(x.l,s,{main:x.h[2]}))}
else if(c==='stale_exact_main'){const x=completeLedger(l,s);z.push(...await validateLifecycle(x.l,s,{head:x.h[0],main:'d'.repeat(40),headDigests:new Set([x.g[0]]),mergeDigests:new Set([x.g[1]]),requireEvidence:true}))}
