...
async function verifyExternalSource(r,context,source){
  ...
  if(r.event_type==='exact_head_validated'){
    ...
    const run=await apiGet(`${prefix}/actions/runs/${r.source.run_id}`),jobs=await apiGet(`${prefix}/actions/runs/${r.source.run_id}/jobs?per_page=100`),artifact=await apiGet(`${prefix}/actions/artifacts/${r.source.artifact_id}`),pr=await apiGet(`${prefix}/pulls/${r.pull_request}`),job=(jobs.jobs||[]).find(x=>x.id===r.source.job_id),prBound=pr.number===r.pull_request&&pr.head?.sha===r.subject_sha&&pr.base?.sha===r.base_sha&&pr.head?.repo?.id===policy.repository.id&&pr.base?.repo?.id===policy.repository.id;
